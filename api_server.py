...
def predict():
    try:
        ...
        return jsonify({'success': True, 'prediction': result['top_prediction'], 'top_predictions': result['top_k_predictions']})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
    ...
    try:
        ...
        for image_file in image_files:
            try:
                ...
                image = Image.open(io.BytesIO(image_bytes)).convert('RGB')
                result = model.predict(image, top_k=top_k)
                results.append({'filename': image_file.filename, 'success': True,
                                'prediction': result['top_prediction'],
                                'top_predictions': result['top_k_predictions']})
            except Exception as e:
                results.append({'filename': image_file.filename, 'success': False, 'error': str(e)})
